...
namespace SolarSystem
{
    class OrbitsCalculator : INotifyPropertyChanged
    {
        ...
        {
            //1.First step - near 10% improvement
            //It is meanless to calculate "this.Days / EarthRotationPeriod" in the loop.
            //This calculation does not depend on loop state
            //2.Second major step
            //It is a mistake to explicit cast decimal to double
            //This was the bottleneck

            var fulRotation = this.Days/EarthRotationPeriod;

            for (double step = 0; step <= 360; step+=0.00005)
            {
                this.EarthRotationAngle = step * fulRotation;
            }
