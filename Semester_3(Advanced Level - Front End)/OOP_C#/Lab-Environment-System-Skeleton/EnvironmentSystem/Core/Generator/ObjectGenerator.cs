    ...
    public class ObjectGenerator : IObjectGenerator<EnvironmentObject>
    {
        ...
        public IEnumerable<EnvironmentObject> SeedInitialObjects()
        {
            ...
            for (int i = 0; i < StaticStarCount; i++)
            {
                ...
                var isFallingStar = this.randomGenerator.Next(0, 30) % 5 == 0 ;

                if (isFallingStar)
                {
                    if (this.randomGenerator.Next(0,2) % 2 == 0)
                    {
                ...
                if (this.randomGenerator.Next(0, 30) % 9 == 0)
                {
                        generatedObjects.Add(new UnstableStar(x, y, 1, 1, new Point(-1, 1)));
