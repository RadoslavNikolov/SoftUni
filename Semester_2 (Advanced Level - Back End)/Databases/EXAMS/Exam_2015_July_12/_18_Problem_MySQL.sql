...
CREATE TABLE `salaries` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `from_value` float(2) NOT NULL,
  `to_value` float(2) NOT NULL,
  PRIMARY KEY (`id`)
);
...
CREATE TABLE `job_ads` (
  ...
  PRIMARY KEY (`id`),
  KEY `fk_job_ads_users` (`author_id`),
  KEY `fk_job_ads_salaries` (`salary_id`),
  CONSTRAINT `fk_job_ads_users` FOREIGN KEY (`author_id`) REFERENCES `users` (`id`),
  CONSTRAINT `fk_job_ads_salaries` FOREIGN KEY (`salary_id`) REFERENCES `salaries` (`id`)
);
...
CREATE TABLE `job_ad_applications` (
  ...
  PRIMARY KEY (`id`),
  KEY `fk_job_ad_applications_job_ads` (`job_ad_id`),
  KEY `fk_job_ad_applications_users` (`user_id`),
  CONSTRAINT `fk_job_ad_applications_job_ads` FOREIGN KEY (`job_ad_id`) REFERENCES `job_ads` (`id`),
  CONSTRAINT `fk_job_ad_applications_users` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`)
);
...
select 
    ...
    ifnull(u.fullname, 'NULL') AS `fullname`,
    ja.title AS `Job`,
    FORMAT(sa.from_value ,2) as `From Value`,
    FORMAT(sa.to_value, 2) as `To Value`
from job_ad_applications jaa
left JOIN job_ads ja ON  jaa.job_ad_id = ja.id
 left JOIN users u ON jaa.user_id = u.id
 left JOIN salaries sa ON sa.id = ja.salary_id
ORDER BY `username`asc,`Job` asc
