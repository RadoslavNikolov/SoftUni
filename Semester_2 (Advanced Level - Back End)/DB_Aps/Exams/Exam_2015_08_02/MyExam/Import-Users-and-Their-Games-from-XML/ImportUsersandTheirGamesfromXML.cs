namespace Import_Users_and_Their_Games_from_XML
{
    ...
    class ImportUsersandTheirGamesfromXML
    {
        ...
        private static void AddUserToGame(User user, XElement xUser, DiabloEntities context)
        {
            ...
            var gamesList = xUser.XPathSelectElements("games/game");

            foreach (var xGame in gamesList)
            {

                var gameName = xGame.Element("game-name").Value;
                var character = xGame.Element("character");
                var characterName = character.Attribute("name");
                var cash = character.Attribute("cash");
                var level = character.Attribute("level");
                var joinedDate = xGame.Element("joined-on").Value;
